...
| `action-desk-canvas` | Use Work, Memory, Tasks, managed Automations and Config in one workspace while preserving local drafts and keeping execution authority separate | Optional | runtime | [Bring work, context and progress together](how-to/commitments-and-action-desk.md#action-desk-canvas) |
...
| `dream` | Reflect on selected Margo session evidence, review candidate interpretations and retrieve relevant sourced episodes next session | Limited | runtime | [Dream over opted-in session checkpoints](how-to/dream.md#reflect-on-one-daily-page) |
...
| `memory-canvas` | Inspect, search and request a foreground correction or forgetting decision from an optional app panel | Optional | runtime | [Inspect memory inside the workspace](how-to/memory-controls-and-learning.md#memory-canvas) |
...
| `setup` | Install Margo and confirm the Work IQ connection works | Implemented | runtime | [Get a first useful brief](how-to/setup-and-migration.md#setup) |
...
| `personalization` | Configure the assistant display name and dedicated work area, plus preferences, VIPs, working hours and drafting voice | Implemented | runtime | [Make Margo work the way you do](personalization.md#personalization) |
